...
const RequestsPage = {
  ...
  // Populate service type dropdown with error handling
  populateServiceTypeDropdown() {
    // Show loading state
    const serviceType = document.getElementById('service-type');
    if (!serviceType) return;
    
    serviceType.innerHTML = '<option value="">Loading service types...</option>';
    ...
    TicketService.getForms()
      .then(forms => {
        ...
        if (forms && Array.isArray(forms) && forms.length > 0) {
          ...
          // Log success
          console.log(`Successfully loaded ${forms.length} service types`);
        } else {
          // Add a message if no forms are available
          options += '<option value="" disabled>No service types available</option>';
          console.warn('No forms available to populate dropdown');
        }
      ...
      .catch(error => {
        ...
        // Provide a fallback UI even on error
        serviceType.innerHTML = '<option value="">Choose a Service Type</option><option value="" disabled>Error loading service types</option>';
        serviceType.disabled = false;
      });
  },
