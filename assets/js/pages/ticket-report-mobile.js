/**
 * Ticket Report Mobile JavaScript
 * Handles functionality for the ticket-report-mobile.html page
 */

// Main object for ticket-report-mobile page functionality
const TicketReportMobilePage = {
  ...
  initDatePickers() {
    ...
    document.addEventListener('DOMNodeInserted', (e) => {
      if (e.target.classList && e.target.classList.contains('datepicker') && !e.target._flatpickr) {
        flatpickr(e.target, {
          dateFormat: 'Y-m-d'
        });
      }
      
      if (e.target.classList && e.target.classList.contains('timepicker') && !e.target._flatpickr) {
        flatpickr(e.target, {
          enableTime: true,
          noCalendar: true,
          dateFormat: 'H:i',
  ...
  loadCategories() {
    ...
    fetch('/app/api/ticket/ticket-category', {
      method: 'GET',
      headers: {
  ...
  loadFormsByCategory(categoryId) {
    ...
    fetch('/app/api/ticket/ticket-forms-with-categories', {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('pwa_token')}`,
        'Accept': 'application/json'
  ...
  loadStatuses() {
    ...
    fetch('/app/api/ticket/ticket-status', {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('pwa_token')}`,
        'Accept': 'application/json'
  ...
  loadFormFields(formId) {
    ...
    fetch(`/app/api/ticket/ticket-form/${formId}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('pwa_token')}`,
        'Accept': 'application/json'
  ...
  renderFormField(field) {
    ...
    switch (field.type) {
      ...
      case 'signature':
        html = `
          <div class="row">
            <div class="label">${field.label || 'Signature'}&nbsp;${required}</div>
            <div class="signature-container">
              <canvas class="signature-canvas signature-hhn0vkbc5a" width="300" height="200"></canvas>
              <div class="signature-buttons">
                <button type="button" class="signature-btn js-sign-signature">Sign</button>
                <button type="button" class="signature-btn js-clear-signature" style="display:none">Clear</button>
              </div>
            </div>
          </div>
        `;
  ...
  preSelectRequestType(requestType) {
    ...
    .then(categories => {
      // Then, load all forms
      return fetch('/app/api/ticket/ticket-forms-with-categories', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('pwa_token')}`,
          'Accept': 'application/json'
    ...
    .then(data => {
      const { categories, forms } = data;
      
      // Find the form that matches the request type
      const matchingForm = forms.find(form => {
        return form.name.toLowerCase().includes(requestType.toLowerCase());
      });
    ...
    this.dynamicFormFields.forEach(field => {
      ...
      switch (field.type) {
        ...
        case 'signature':
          ...
          const signatureCanvas = document.querySelector(`.signature-hhn0vkbc5a`);
          if (signatureCanvas) {
            const signaturePad = new SignaturePad(signatureCanvas);
            if (!signaturePad.isEmpty()) {
              formData[field.name] = signaturePad.toDataURL();
            }
          }
          break;
      }
